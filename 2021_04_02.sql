...
1. 시퀀스명.CURRVAL : '시퀀스'의 현재 값 반환
-- 시퀀스가 생성되고 해당 세션의 첫 번째 명령은 반드시 시퀀스명.NEXTVAL이어야 한다.
...
FROM DUAL;
-- 에러가 난다 (값을 지칭할 수 없음) : 자기자신은 자신을 지칭할 수 없다 / 지칭을 해줄곳을 만들어야한다
...
사용예] 오늘이 2005년 7월 28일인 경우 'm001'회원이 제품 'P201000004'을 5개 구입했을때 CART테이블에 해당
자료를 삽입하는 쿼리를 작성하시오
...
- 별도의 추가 공간이 필요하고 INDEX FILE을 위한 PROCESS가 요구된다
