...
-블록(BLOCK)구조로 구성
-미리 컴파일 되어 실행 가능한 상태로 서버에 저장되어 필요시 호출되어 사용됨
-모듈화, 캡슐화 기능 제공
-Anonymous Block, Stored Procedure, User Defined Function, Package, Trigger
등으로 구성
...
1)변수, 상수 선언
  ...
  - REFERENCES 변수 - 해당 테이블의 컬럼이나 행의 대응하는 타입과 크기를 참조하는 변수
  - COMPOSITE 변수 - PL/SQL에서 사용하는 배열 변수
  RECORD TYPE
  TABLE TYPE 변수
  - BIND 변수 파라메터로 넘겨지는 IN, OUT, INOUT에서 사용되는 변수 RETURN 
  되는 값을 전달 받기 위한 변수
